...
import datetime
import os
parser = argparse.ArgumentParser(description = 'Flow_Generator')
...
"""
    保存
"""
parser.add_argument('--pattern', type=str, default='test', #train or test
...
save_path = os.path.join('./flow_result/' + time_str)
...
# loss_weight
parser.add_argument('--weight_regular_loss', type=float, default=0,
                    )
    # temp_best 4 * 1e-7
parser.add_argument('--weight_MSE_loss', type=float, default=10.0,
                   )
parser.add_argument('--weight_edge_loss', type=float, default=0,
                   )
...
'''
    网络初始化
'''
parser.add_argument('--module', type=str, default= 'INR', #INR or U-NET
